...
if __name__ == '__main__':
    #------------Path of the images --------------------------------------------------------------
    original_imgs_train = "./data/STARE/images/"
    groundTruth_imgs_train = "./data/STARE/1st_labels_ah/"
    borderMasks_imgs_train = "./data/STARE/mask/"
    #---------------------------------------------------------------------------------------------
    dataset_path = "./data/STARE4_data/"
    if not os.path.isdir(dataset_path):
        os.mkdir(dataset_path)
    #getting the training and valid datasets
    val_rate = 0.25
    file_list = sorted(os.listdir(original_imgs_train))
    split_pos = int(len(file_list) * (1 - val_rate))
    train_list = file_list[:0]+file_list[5:]
    val_list =  file_list[0:5]
